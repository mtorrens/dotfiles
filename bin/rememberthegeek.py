...
def displayList(listTitle, tasks):
	s = "%s:" % listTitle
	print colored(s.encode("utf-8"), attrs=['underline'])
	if(len(tasks) == 0):
		print "  Nothing to do!"
	else:
		for task in tasks:
			...
			overdue = False
			if (task["due"] != "never"):
				# Figure out when this thing is due
				try:
					d = datetime.strptime(task["due"], "%a %d %b %y")
					now = datetime.now()
					delta = d - now
					if (delta.days < 0):
						overdue = True
						duestr = "%d/%d!" % (d.month, d.day)
					else:
						if (delta.days > 5):
							duestr = "%d/%d" % (d.month, d.day)
						else:
			...
			if (overdue):
				print colored(task["title"].encode("utf-8"), attrs=['bold', 'underline'])
			else:
				print colored(task["title"].encode("utf-8"), attrs=['bold'])
			
			print "   ",
			print duestr.ljust(10).encode("utf-8"),
