...
(defvar *textmate-mode-map*
  (let ((map (make-sparse-keymap)))
    (cond (t
    (define-key map [C-return] 'textmate-next-line)
    (define-key map (kbd "C-M-t") 'textmate-clear-cache)
    (define-key map (kbd "C-M-]") 'align)
    (define-key map (kbd "C-M-[") 'indent-according-to-mode)
    (define-key map (kbd "C-]")  'textmate-shift-right)
    (define-key map (kbd "C-[") 'textmate-shift-left)
    (define-key map (kbd "C-/") 'comment-or-uncomment-region-or-line)
    (define-key map (kbd "C-L") 'textmate-select-line)
    (define-key map (kbd "C-t") 'textmate-goto-file)
    (define-key map (kbd "C-T") 'textmate-goto-symbol)
